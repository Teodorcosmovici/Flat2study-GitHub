...
export function UnplacesBookingWidget({ listing, onBookingRequest, onDatesChange }: UnplacesBookingWidgetProps) {
  ...
  return (
      <Card className={cn(
        "sticky top-24",
        isMobile && "fixed -bottom-20 left-0 right-0 z-50 rounded-t-xl rounded-b-none border-t shadow-2xl bg-background/95 backdrop-blur-md"
      )}>
              ...
              <div>
                ...
                <Popover open={showCheckInCalendar} onOpenChange={setShowCheckInCalendar}>
                  <PopoverTrigger asChild>
                    ...
                    <PopoverContent side="top" sideOffset={-260} avoidCollisions={false} className="w-auto p-0 bg-background/95 backdrop-blur-sm border shadow-lg z-[60]" align="start">
                    <Calendar
                      mode="single"
                      selected={checkIn}
