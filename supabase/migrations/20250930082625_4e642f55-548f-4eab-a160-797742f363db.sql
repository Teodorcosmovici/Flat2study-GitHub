-- Create RPC function to get pending booking requests with tenant and listing information
CREATE OR REPLACE FUNCTION public.get_pending_bookings_with_tenant(p_landlord_id uuid)
RETURNS TABLE(
  id uuid,
  listing_id uuid,
  tenant_id uuid,
  ...
  landlord_response_due_at timestamp with time zone,
...
AS $function$
  ...
  WHERE b.landlord_id = p_landlord_id
    ...
    AND b.landlord_response IS NULL
    AND EXISTS (
